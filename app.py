...
class Config:
    ...
    athena_output_s3: str = (
        "s3://fitbit-dashboard/athena-logs"  # Athenaクエリ結果の出力先S3バケット
    )
...
def load_data_athena(
    category: str, config: Config, start_date: str = None, end_date: str = None
) -> pd.DataFrame:
    """
...
def filter_data_by_date(
    df: pd.DataFrame, selected_start: datetime.date, selected_end: datetime.date
) -> pd.DataFrame:
    """DataFrameを選択された日付範囲でフィルタします。"""
    return df[
        (df["date"].dt.date >= selected_start) & (df["date"].dt.date <= selected_end)
    ].copy()


def plot_category_data(
    category: str, df: pd.DataFrame, config: Config, window: int = 30
) -> None:
    """指定カテゴリのデータをグラフ表示します。"""
    metric = config.metric_columns[category]
...
for i, category in enumerate(categories):
    with cols[i]:
        avg_metric = calculate_weekly_average(
            data[category], config.metric_columns[category]
        )
        st.metric(
